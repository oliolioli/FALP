...
%scalar_product_acc(++L1,++L2,++Acc,-P)
scalar_product_acc([H1|T1], [H2|T2], Acc, P) :-
  TemporaryResult is H1*H2,
  NewAcc is Acc + TemporaryResult,
  scalar_product_acc(T1, T2, NewAcc, P).

scalar_product_acc([], [], Acc, Acc).
...
fibonacci_acc(0, _, Acc, Acc) :- !.  % return Acc, not Prev
...
count_occur(X, [Head|Tail], N) :-
  count_occur(X, Tail, M),
    (   var(Head)
    ->  N is M + 1
    ;   (Head == X -> N is M + 1 ; N = M)         % this is needed to pass the test with 5 empty arguments.
    ).
...
count_occur_acc(X, [Head|Tail], Acc ,N) :-
  (   var(Head)
  ->  NewAcc is Acc + 1
  ;   (Head == X -> NewAcc is Acc + 1 ; NewAcc = Acc)
  ),
